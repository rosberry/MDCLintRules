 ...
 */
class MaterialSliderDetector : ResourceXmlDetector(), SourceCodeScanner {

    override fun appliesTo(folderType: ResourceFolderType): Boolean = folderType == ResourceFolderType.LAYOUT

    override fun getApplicableElements(): Collection<String>? =
            listOf("androidx.appcompat.widget.AppCompatSeekBar", "SeekBar")
    ...
    private fun reportUsage(context: XmlContext, node: Node) {
        val fix = fix()
            ...
            .replace()
            .text(node.nodeName)
            .with("com.google.android.material.slider.Slider")
            .autoFix()
            .build()
    ...
    companion object {
        ...
        private val IMPLEMENTATION = Implementation(MaterialSliderDetector::class.java, Scope.RESOURCE_FILE_SCOPE)

        val ISSUE: Issue = Issue
            .create(
                    id = "MaterialSliderDetector",
                    briefDescription = "Use MDC Component",
                    explanation = "You should use Slider".trimIndent(),
