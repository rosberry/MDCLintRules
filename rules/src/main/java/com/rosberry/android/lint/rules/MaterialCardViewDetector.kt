 ...
 */
class MaterialCardViewDetector : ResourceXmlDetector(), SourceCodeScanner {

    override fun appliesTo(folderType: ResourceFolderType): Boolean = folderType == ResourceFolderType.LAYOUT

    override fun getApplicableElements(): Collection<String>? =
            listOf("androidx.cardview.widget.CardView")
    ...
    private fun reportUsage(context: XmlContext, node: Node) {
        val fix = fix()
            ...
            .replace()
            .text(node.nodeName)
            .with("com.google.android.material.card.MaterialCardView")
            .autoFix()
            .build()
    ...
    companion object {
        ...
        private val IMPLEMENTATION = Implementation(MaterialCardViewDetector::class.java, Scope.RESOURCE_FILE_SCOPE)

        val ISSUE: Issue = Issue
            .create(
                    id = "MaterialCardViewDetector",
                    briefDescription = "Use MDC Component",
                    explanation = "You should use MaterialCardView".trimIndent(),
